...
const ListingHeader = ({
  ...
  const currentSortLabel = sortOptions.find(option => option.id === currentSort)?.label || "Relevance";
    ...
    <div className="flex flex-col xs:flex-row xs:items-center justify-between w-full mb-4 gap-2 xs:gap-4">
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-sm text-neutral-500">{count.toLocaleString()} properties</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 xs:gap-4 mt-2 xs:mt-0">
        ...
        <div className="flex border rounded-md overflow-hidden">
          <button 
            ...
            className={`p-1.5 xs:p-2 ${currentView === "compact" ? "bg-gray-100" : "bg-white"}`}
          ...
          <button 
            ...
            className={`p-1.5 xs:p-2 ${currentView === "list" ? "bg-gray-100" : "bg-white"}`}
          ...
          <button 
            ...
            className={`p-1.5 xs:p-2 ${currentView === "grid" ? "bg-gray-100" : "bg-white"}`}
          ...
          <DropdownMenuContent align="end" className="w-56">
            {sortOptions.map((option) => (
              <DropdownMenuItem 
                ...
                className={`cursor-pointer ${currentSort === option.id ? "bg-gray-100 font-medium" : ""}`}
              >
                {option.label}
              </DropdownMenuItem>
            ))}
